...
public class MarvelPaths {	
	public static void main(String[] args) throws MalformedDataException {
		...
		/*
		Edge<String, String> hawk = graph.getEdges("HAWK");
		Edge<String, String> capt = graph.getEdges("CAPTAIN AMERICA");
		Edge<String, String> skull = graph.getEdges("OLD SKULL");
		int skullToHawk = skull.getLabels("HAWK").size();
		int hawkToCapt = hawk.getLabels("CAPTAIN AMERICA").size();
		int hawkToPharoah = hawk.getLabels("PHARAOH RAMA-TUT").size();
		int captToPharoah = capt.getLabels("PHARAOH RAMA-TUT").size();
		
		System.out.println("skullToHawk = " + skullToHawk);
		System.out.println("hawkToCapt = " + hawkToCapt);
		System.out.println("hawkToPharoah = " + hawkToPharoah);
		System.out.println("captToPharoah = " + captToPharoah);
		double expectedCost = ((double) 1 / skullToHawk) + ((double) 1 / hawkToCapt) + ((double) 1 / captToPharoah);
		double actualCost = ((double) 1 / skullToHawk) + ((double) 1 / hawkToPharoah);
	...
	public static Graph<String, String> createGraph(String fileName) throws MalformedDataException {
		...
		for (String character : characters) {
			graph.addNode(character, EdgeSet.class);
		}
	...
	public static List<BookEdge<String, String>> findPath(Graph<String, String> graph, String start, String dest) {
		...
		while (!toVisit.isEmpty()) {
			...
			Edge<String, String> edges = graph.getEdges(currNode);
			...
			for (String child : children) {
				if (!nodeToPath.containsKey(child)) {
					...
					TreeSet<String> books = new TreeSet<String>(edges.getLabels(child));
