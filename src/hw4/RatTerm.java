...
public final class RatTerm {
  ...
  public boolean isZero() {
  	// Bug: when checking to see if coeff = 0, access RatNum zero, not RatTerm zero.
  ...
  public RatTerm add(RatTerm arg) {
  	// Exception handler can be made into private helper method 
    if ((this.expt != arg.expt) && (!this.isNaN() && !this.isZero()) && 
    		(!arg.isNaN() && !arg.isZero()) ) {
    	throw new IllegalArgumentException();
    } else if (arg.isNaN() || this.isNaN()) {
    	return NaN;
    } else if (arg.isZero()) {
